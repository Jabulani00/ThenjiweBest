...
@Component({
  ...
  templateUrl: './add-storeroom.page.html',
  styleUrls: ['./add-storeroom.page.scss'],
})
export class AddStoreroomPage implements OnInit {
  ...
  async addItem() {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        this.firestore.collection('storeroom').doc(docId).update({
          name: this.itemName,
          category: this.itemCategory,
          description: this.itemDescription,
          imageUrl: this.imageUrl || '',
      ...
      this.presentToast('Item added to cart','success');
      await this.firestore.collection('storeroom').add(newItem);
      this.clearFields();
    } catch (error) {
